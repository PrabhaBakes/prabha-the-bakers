...
# Contact + UPI configuration.
CONTACT_PHONE = "8869946488"
UPI_ID = os.getenv("UPI_ID", "")
UPI_NAME = os.getenv("UPI_NAME", "Prabha The Bakers")
...
def admin_settings():
    ...
    st=load_settings()
    for k in ["brand_name","tagline","phone","upi_id","upi_name","upi_qr","hero_eyebrow","hero_title","hero_description","hero_image","background_image","footer_tagline","gold"]:
...
def admin_single_image(slot):
    if not require_admin(): return redirect(url_for("admin"))
    allowed={"hero_image","background_image","story_image","video_image","upi_qr"}
...
def create_order():
    ...
    return jsonify({"order_id":order_id,"amount":total,"upi_id":upi_id,"upi_name":upi_name,"upi_qr":settings.get("upi_qr", ""),"phone":settings.get("phone",CONTACT_PHONE),"items":clean_items})

@app.route("/api/order-confirm", methods=["POST"])
def order_confirm():
    data=request.get_json(silent=True) or {}
    if not data.get("order_id"):return jsonify({"ok":False,"error":"Missing order ID."}),400
    # Customer return confirmation only; this does not verify bank settlement.
    return jsonify({"ok":True,"order_id":data["order_id"]})
